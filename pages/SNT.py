...
########################################################################################################
        ...
        # Display and provide download link for Excel file
        st.download_button(label='Download First Month Reported Excel File',
                           data=excel_data,
...
if analysis_option == "Geospatial Analysis":
    ...
    if uploaded_file:
        ...
        df['adm3_uid'] = df.groupby(['adm1', 'adm2', 'adm3']).ngroup()
        # Processing steps as in the original code
        cols = ['susp','test', 'conf', 'maltreat']
        df.insert(len(df.columns), 'key_variables', df[cols].sum(axis=1, skipna=True, min_count=1))
        df['key_variables'] = df['key_variables'].fillna(0)
        df.insert(len(df.columns), 'reported', np.where(df.key_variables == 0, 0, 1))

        d = {}
        for hf in sorted(df.hf_uid.unique()):
            t = df[df.hf_uid == hf]
            d[hf] = t[t.reported == 1].YM.min()
        df.insert(len(df.columns), 'first_month_reported', df.hf_uid.map(d))
        df.insert(len(df.columns), 'reported_detail', df.apply(lambda x: 0.5 if ((x.reported == 0) & (x.YM > d[x.hf_uid])) else x.reported, axis=1))
        df.insert(len(df.columns), 'hf_active', np.where(df.reported_detail == 0, False, True))
        # Calculating confirmed variable
        reporting_monthly_adm3 = {}
        for chiefdom in df.adm3_uid.unique():
            t = df[df.adm3_uid == chiefdom]
            t = (t[['YM', 'hf_uid', 'hf_active']]
                 .groupby(['YM'])['hf_active'].sum()
                 .to_frame()
                 .reset_index()
        ...
        t = df[['adm3_uid', 'hf_uid']].drop_duplicates()
        t = t.groupby(['adm3_uid'])['hf_uid'].count().reset_index()
        d_chiefdoms = dict(zip(t.adm3_uid, t.hf_uid))
        ...
        for denominator in ['ActiveHFsDenominator']:
            ...
            df0['conf_reported'] = np.where(df0['conf'].isnull(), False, True)

            if denominator == 'AllHFsDenominator':
                df0.insert(0, 'denominator', df0.adm3_uid.map(d_chiefdoms))
            else:
                def temp(x):
                    t = reporting_monthly_adm3[x.adm3_uid]
                    return t[t.YM == x.YM].denominator.values[0]

                df0.insert(0, 'denominator', df0.apply(lambda x: temp(x), axis=1))



        # Step 2: Group by adm3 and Year, Create DataFrame for Each Year
        grouped_df = df0.groupby(['adm1', 'adm2', 'adm3','adm3_uid','Year']).sum().reset_index()
        grouped_df['conf_RR'] = 100 * grouped_df['conf_reported'].div(grouped_df['hf_active'])
        grouped_df['TPR']= 100 * grouped_df['conf'].div(grouped_df['test'])
