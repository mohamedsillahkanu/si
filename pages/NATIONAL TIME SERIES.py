...
# Variable selection (assuming these are the variables available in the DataFrame)
...
if uploaded_shp and uploaded_shx and uploaded_dbf and years and months and variable:
    ...
    # Line plot for the selected variable
    st.subheader("Line Plot for Selected Variable")

    # Prepare DataFrame for line plot
    line_plot_df = combined_df.groupby(['Year', 'Month'])[variable].mean().reset_index()

    fig, ax = plt.subplots()
    for year in years:
        year_data = line_plot_df[line_plot_df['Year'] == year]
        ax.plot(year_data['Month'], year_data[variable], marker='o', label=f'Year {year}')
    
    ax.set_title(f"{variable} over Months")
    ax.set_xlabel('Month')
