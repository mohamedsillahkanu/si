    ...
    if selected_categories:
        ...
        if st.checkbox("Select Colors for Categories"):
            for i, category in enumerate(selected_categories):
                color_mapping[category] = st.color_picker(f"Select Color for '{category}' in {map_column}:", value=color_mapping[category])
            ...
            try:
                ...
                if map_column not in merged_gdf.columns:
                    ...
                    if merged_gdf[map_column].isnull().sum() > 0:
                        ...
                        missing_handle = Patch(color=missing_value_color.lower(), label=f"{missing_value_label} ({merged_gdf[map_column].isnull().sum()})")
                    ...
                    map_image_path = f"{image_name}.png"
                    fig.savefig(map_image_path, bbox_inches="tight", dpi=300)
                    st.download_button("Download the Map", map_image_path, file_name=f"{image_name}.png", mime="image/png")
