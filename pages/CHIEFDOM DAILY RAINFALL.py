...
def process_chirps_data(gdf, year, month):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        with rasterio.open(unzipped_file_path) as src:
            ...
            # Flatten the masked array and capture daily data for each geometry
            daily_data = []
...
variable = st.selectbox("Select Variable for Line Plot", ["daily_rain"])  # Daily rainfall
...
if uploaded_shp and uploaded_shx and uploaded_dbf and years and months and variable:
    ...
    st.write(combined_df[['geometry', 'daily_rain', 'Year', 'Month', 'FIRST_DNAM', 'FIRST_CHIE']])
    ...
    for dnam in combined_df['FIRST_DNAM'].unique():
        ...
        for chie in dnam_data['FIRST_CHIE'].unique():
            ...
            # Plot daily rainfall data without aggregation
            fig, ax = plt.subplots(figsize=(12, 8))
            for year in years:
                for month in months:
                    month_data = chie_data[(chie_data['Year'] == year) & (chie_data['Month'] == month)]
                    for daily_rain in month_data['daily_rain']:
                        ax.plot(range(len(daily_rain)), daily_rain, label=f'{year}-{month:02d}')

            ax.set_title(f"{chie} in {dnam}: Daily {variable}")
            ax.set_xlabel('Days')
