...
def create_variable_plots(df, hf_id, year, variable, plot_title, y_label):
    df_filtered = df[df['hf_uid'] == hf_id]
    df_filtered = df_filtered[df_filtered['year'] == year]
    
    if len(df_filtered) == 0:
        st.error(f"No data available for facility {hf_id} in year {year}")
        return
    ...
    for i, method in enumerate(correction_methods, 1):
        ...
        if method in df_filtered.columns:
            ...
            # Add IQR bounds
            fig.add_hline(y=lower_bound, line=dict(color="green", dash="dash"), row=row, col=col)
            fig.add_hline(y=upper_bound, line=dict(color="red", dash="dash"), row=row, col=col)
    ...
    for i in range(8):
        fig.update_yaxes(title_text=y_label, row=(i//2)+1, col=(i%2)+1)
...
if uploaded_file:
    ...
    with col1:
        ...
        # Get available years for selected facility
        years = sorted(df[df['hf_uid'] == selected_facility]['year'].unique())
        selected_year = st.selectbox("Select Year", years)
    
    with col2:
        plot_title = st.text_input("Enter Plot Title", "Variable Analysis")
        y_label = st.text_input("Enter Y-axis Label", "Value")
    
    variables = ["allout", "susp", "test", "conf", "maltreat", "pres", "maladm", "maldth"]
    selected_var = st.selectbox("Select Variable", variables)
    
    create_variable_plots(df, selected_facility, selected_year, selected_var, plot_title, y_label)
