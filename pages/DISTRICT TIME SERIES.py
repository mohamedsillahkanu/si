...
if uploaded_shp and uploaded_shx and uploaded_dbf and years and months and variable:
    ...
    st.write(combined_df[['geometry', 'mean_rain', 'Year', 'Month', 'FIRST_DNAM']])

    # Line plot for each unique FIRST_DNAM value
    st.subheader("Line Plots for Each FIRST_DNAM")

    # Create separate figures for each unique FIRST_DNAM
    for name in combined_df['FIRST_DNAM'].unique():
        name_data = combined_df[combined_df['FIRST_DNAM'] == name]
