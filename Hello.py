...
if uploaded_file:
    # UI for column selections and customizations
    ...
    if st.button("Generate Map"):
        try:
            ...
            if map_column not in merged_gdf.columns:
                ...
                legend_elements = [Patch(facecolor=color_mapping[cat], label=cat) for cat in selected_categories]
                ax.legend(handles=legend_elements, title=legend_title, title_fontsize='13', loc='center left', bbox_to_anchor=(1, 0.5))

                st.pyplot(fig)
                
                # Provide download link for the map image
                fig.savefig(f'/tmp/{image_name}.png')
                with open(f'/tmp/{image_name}.png', 'rb') as file:
                    st.download_button("Download Map Image", file, file_name=f"{image_name}.png", mime="image/png")
