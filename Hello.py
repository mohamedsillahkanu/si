...
# UI elements for selecting columns, titles, and map settings
...
map_column = st.selectbox("Select Map Column:", [col for col in df.columns if col not in ['FIRST_DNAM', 'FIRST_CHIE', 'adm3']])
...
# Create color dropdown with swatches
def color_dropdown(label, options, default):
    st.write(f"{label}:")
    selected = st.selectbox(
        "",
        options=options,
        format_func=lambda x: f'<div style="background-color:{x}; width: 100px; height: 20px; display: inline-block; border: 1px solid #ddd;"></div>',
        key=label
    )
    return selected
...
if st.checkbox("Select Colors for Categories"):
    color_options = [color_mapping[cat] for cat in selected_categories]
    for category in selected_categories:
        color_mapping[category] = color_dropdown(f"Select Color for '{category}' in {map_column}", color_options, color_mapping[category])
...
if st.button("Generate Map"):
    try:
        ...
        if map_column not in merged_gdf.columns:
            ...
            # Add boundaries if columns selected
            ...
            st.download_button(
                ...
                data=open(f"/tmp/{image_name}.png", "rb").read(),
