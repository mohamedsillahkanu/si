        ...
        if st.button("Generate Map"):
            try:
                ...
                if map_column not in merged_gdf.columns:
                    ...
                    custom_cmap = ListedColormap([color_mapping[cat] for cat in ordered_categories])
                    ...
                    # Customize the legend with rectangular boxes and bold text
                    handles = [Patch(facecolor=color_mapping[cat], edgecolor='black', label=cat) for cat in ordered_categories]
                ...
                st.error(f"Error: The specified column '{e.args[0]}' does not exist in the merged dataset.")
else:
